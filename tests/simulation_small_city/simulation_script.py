...
def predict_route(latest_fill_dict, day):
    ...
    files = {
        "latest_data_file": ("latest_fill.json", latest_data_json, "application/json"),
        "start_node": (None, "Node1"),  # Form field (text)
        "day_name": (None, f"Day_{day}")  # Form field (text)
    }
...
def simulate_daily_cycle(num_days=10):
    ...
    for day in range(num_days):
        ...
        route, round_cost, round_reward = predict_route(latest_fill, day)
        ...
        if round_cost == -1 or round_reward == -1:
            print(f"  [ERROR] Invalid prediction response. Cost: {round_cost}, Reward: {round_reward}")
            continue
